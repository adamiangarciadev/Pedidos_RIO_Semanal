...
function renderPedido(){
  $('#count').textContent = pedido.length;
  const w = $('#pedidoWrap');
  if(pedido.length===0){ w.innerHTML = `<div class="empty">Todavía no agregaste combinaciones. Armá el pedido y aparecerá acá.</div>`; return; }
  // Tabla
  let html = `<div style="overflow:auto"><table><thead><tr>
    <th>Código</th><th>Descripción</th><th>Talle</th><th>Cant.</th><th>Promo</th><th>Sucursal</th><th></th>
  </tr></thead><tbody>`;
  pedido.forEach((it,idx)=>{
    html += `<tr>
      <td style="font-weight:600">${it.articulo}</td>
      <td class="muted">${it.desc}</td>
      <td>${it.talle}</td>
      <td>${it.cantidad}</td>
      <td class="muted">${it.promo}</td>
      <td class="muted">${it.sucursal}</td>
      <td><button class="btn icon" onclick="delItem(${idx})">✕</button></td>
    </tr>`
  });
